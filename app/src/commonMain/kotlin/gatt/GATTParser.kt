...
class ISOParser(var bytes : ByteArray) {
    ...
    fun <T : GATTValue?> dropThen(bytesToDrop : Int, fn : () -> T?) : T? {
        take(bytesToDrop)
        return fn()
    }
     ...
     */
    fun <T : GATTValue?> onCondition(condition : Boolean, fn : () -> T?) : T? =
        if(condition) fn()
        else null
     ...
     */
    val uint8 = {
            GATTValue.UInt8(take(1).first())
    }
    val sint8 = {
        GATTValue.SInt8(take(1).first())
    }

    val utf8 = {
        nrOfBytes : Int ->
        GATTValue.UTF8(take(nrOfBytes))
    }
     ...
     */
    val flag : (Boolean) -> GATTValue.Flag = {
            bool : Boolean ->
        GATTValue.Flag(bool)
    }
